  ...
  task :import => :environment do
    ...
    exercises.each do |exercise|
      unless exercise["name"] == nil
        e = Exercise.create(name: exercise["name"], youtube_url: exercise["youtube_url"], proper_form_text: exercise["proper_form_text"], is_run: (exercise["is_run"] == "yes" ? true : false), requires_gym: (exercise["requires_gym"] == "yes" ? true : false))
        (1..5).each do |inc|
